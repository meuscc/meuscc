...
const navsStyles = [styles.nav1, styles.nav2, styles.nav3, styles.nav4, styles.nav5];
    ...
    <ul className={''}>
      {navs.map((nav: any) => (
        <li key={nav.name} className={navsStyles[level]}>
          {nav.href ? (
            <a target={'_blank'} rel="noreferrer" href={nav.href}>
          ...
          {nav.children?.length > 0 ? createNavs(nav.children, level + 1) : <></>}
        </li>
      ))}
    </ul>
  );
